...
            }, "script", head);
            var f =
                "qzone,tsina,tqq,weixin,renren,kaixin001,evernote,linkedin,douban,ydnote,xiaoyou,msn,fb,twitter,tieba,baidu,google",
...
                            ifsrc = 'http://open.qzone.qq.com/like?url=' + encodeURIComponent(likeurl) + '&type=' + _gv(
                                o.type, 'button_num')
                        } else {
                            ifsrc = 'http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_likeurl?url=' +
                                encodeURIComponent(likeurl) + '&showcount=' + _gv(o.showcount, 1) + '&style=' + _gv(o.style,
                                2)
                        }
                    } else if (likeid == 'renren') {
                        var n = _gv(o.pageid, false);
                        likeurl = n ? ("http://page.renren.com/" + n) : likeurl;
                        ifsrc = 'http://www.connect.renren.com/like?url=' + encodeURIComponent(likeurl) + '&showfaces=' +
                            _gv(o.showfaces, 'false')
                    } else if (likeid == 'kaixin001') {
                        ifsrc = 'http://www.kaixin001.com/like/like.php?url=' + encodeURIComponent(likeurl) +
                            '&show_faces=' + _gv(o.show_faces, 'false')
...
                    }
                    if (ifsrc) {
                        ci.innerHTML = '<span class="jiathis_txt jiathis_separator jialike"><iframe src="' + ifsrc +
                            '" allowTransparency="true" scrolling="no" border="0" frameborder="0" style="width:' + _gv(
                            o.width, 200) + 'px;height:' + _gv(o.height, 38) + 'px;' + mt + '"></iframe></span>'
                    } else {
                        if (likeid == 'tsinat' || likeid == 'tsina') {
                            if (likeid == 'tsinat') {
                                ci.innerHTML =
                                    '<span class="jiathis_txt jiathis_separator jialike"><wb:like type="text"></wb:like></span>'
                            } else {
                                ci.innerHTML =
                                    '<span class="jiathis_txt jiathis_separator jialike"><wb:like type="number"></wb:like></span>'
                            }
                            var q = ['og:type=webpage', 'og:url=' + encodeURIComponent(url) + '', 'og:title=' +
                                    encodeURIComponent(title) + '', 'og:image=' + encodeURIComponent(pic) + '',
                                    'og:description=' + encodeURIComponent(p) + ''];
...
                        if (tl || w) {
                            w = w ? w : v[0];
                            ci.innerHTML = '<span class="jiathis_txt jiathis_separator jtico jtico_' + webid + '"' + y +
                                '>' + w + '</span>'
                        } else {
...
            if (q && f) {
                s = 'rsc=' + q + '&rnm=' + parseInt(JIATHIS_CONFIGS.uid) + '&rfh=' + E(o.host) + '&rfp=' + E(o.path) +
                    '&pre=' + E(Y) + '&tit=' + escape(T);
...
            var a = navigator.userAgent.toLowerCase().match(
                /ucweb|ios|mobile|ipad|ipod|blackberry|motorola|YahooSeeker|symbian|nokia|android|iphone os|windows ce|rv:1.2.3.4|midp/i);
...
                S = conf.summary ? '&summary=' + encodeURIComponent(conf.summary) : (a ? '&summary=' +
...
                                top = (p.t + T.offsetHeight + div.offsetHeight > wh.h + tl.t ? p.t - div.offsetHeight :
                                    p.t + T.offsetHeight) + "px";
                                left = p.l + "px";
                                T.style.display = a
                            }
                            with(iframe.style) {
...
                list = d.getElementById("jiathis_sharelist"), clist = list.cloneNode(true), h = clist.getElementsByTagName(
                    "input");
...
                for (var p in texts) {
                    eval("var f = /" + (s || ".") + "/ig.test(p)"); !! texts[p].cloneNode && (f && frag.appendChild(
                        texts[p].cloneNode(true)))
                }
                list.innerHTML = "";
...
        d.write('<script type="text/javascript" src="' + JIATHIS_CONFIGS.codehost +
...
        cu = JIATHIS_CONFIGS.custom[a] || {}, U = String(c.url || document.location),
        W = "?webid=" + a,
        G = "&url=" + d(U),
...
        innerhtml = '<img src="' + f +
            '" style="margin-top:25px;" width="129" alt="二维码加载失败" height="129" id="jiathis_follow_img">';
        width = 'width:300px;';
        height = 'height:300px;';
        bh = 'height:181px;';
...
    }
    g.innerHTML =
        '<div id="jiathis_weixin_modal" style="background-clip: padding-box;background-color: #FFFFFF;border: 1px solid rgba(0, 0, 0, 0.3);  border-radius: 6px 6px 6px 6px;  box-shadow: 0 3px 7px rgba(0, 0, 0, 0.3); left: 50%; ' +
        mt + 'overflow: hidden; position: fixed; top: 50%; ' + width + height +
        ' overflow:hidden;" class="jiathis_weixin_modal"><div class="jiathis_modal_header" id="jiathis_modal_header" style="border-bottom: 1px solid #EEEEEE; padding: 9px 15px;"><a style="text-decoration:none;  margin-top: 2px; color: #000000; float: right;  font-size: 20px;  font-weight: bold; cursor:pointer;line-height: 20px; opacity: 0.2; text-shadow: 0 1px 0 #FFFFFF;"class="jiathis_weixin_close" id="jiathis_weixin_close" onclick="jiathis_cancel()"target="_self">×</a><h3 id="jiathis_weixin_h3"style=" line-height: 30px; margin: 0; font-weight:normal; font-family:"微软雅黑";">' +
        wt + '</h3></div><div class="jiathis_modal_body"id="jiathis_modal_body"style="text-align:center;' + bh +
        '"><p id="jiathis_webchat">' + innerhtml +
